...
JPA entities. The `embedding vector(512)` column and the HNSW cosine index
are Postgres-only and therefore not exercised in unit tests — vector storage
and similarity queries are covered by the WireMock-backed Voyage client tests
and the pgvector literal-format unit tests, but the JDBC SQL itself runs only
against a real Postgres + pgvector installation.
